...
module Container = [%styled {|display:flex; flex-flow: column nowrap; align-items:center|}];
let container = [%css [display(`flex), flexFlow(`column, `nowrap), alignItems(`center)]];
// let container = [ %styled {|display:flex, align-items:center|}];
...
let text = (~size) => [%styled
  [
    ...
    transitions([("font-size", `ms(100), `easeInOut, `ms(0))]),
    /* Complex selector that uses .container class defined above */
    /* Rendered as: `.container:hover .text {...}` */
    // select({j|.$container:hover &|j}, [fontSize(Calc.((`px(size) + `pct(150.)) * `n(1.5)))]),
    /* @media quiery with nested selectors */
    media("(max-width: 900px)", [color(`hex("ff69b4")), select(":hover", [color(`hex("fff"))])])
  ]
];
...
/* Compose things */
let smallText = [%styled [fontSize(`em(0.8))]];

let note = css(~extend=smallText, [label("note"), margin2(`px(20), `zero)]);
...
let gridItem = [%styled [padding(`px(10)), color(`hex("fff")), backgroundColor(`hex("29d"))]];

let gridItem1 = [%styled [gridArea("a")]];